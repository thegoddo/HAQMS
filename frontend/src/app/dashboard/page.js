...
export default function Dashboard() {
  ...
  // Debounced search value to avoid firing fetch on every keystroke
  ...
  const fetchPatients = async (page = 1) => {
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE_URL}/patients?page=${page}&limit=5&search=${encodeURIComponent(debouncedSearch)}&gender=${patientGender}`,
        {
  ...
  // Debounce patientSearch to avoid fetch on every keypress
  useEffect(() => {
    const id = setTimeout(() => setDebouncedSearch(patientSearch), 300);
  ...
  }, [debouncedSearch, patientGender, user?.role]);
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 sm:p-8">
        ...
        <div className="flex border-b border-slate-200 dark:border-slate-800 mb-8 overflow-x-auto gap-4">
          {user.role === "ADMIN" && (
            <>
              <button
                onClick={() => setActiveTabOverride("reports")}
              ...
              <button
                onClick={() => setActiveTabOverride("physicians")}
          ...
          {(user.role === "RECEPTIONIST" || user.role === "ADMIN") && (
            <>
              <button
                onClick={() => setActiveTabOverride("patients")}
              ...
              <button
                onClick={() => setActiveTabOverride("book")}
          ...
          {user.role === "DOCTOR" && (
            <>
              <button
                onClick={() => setActiveTabOverride("appointments")}
